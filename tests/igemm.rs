    ...
    for (index, elt) in enumerate(&c1) {
        ...
        if irem != 0 && jrem != 0 {
            if elt.is_nan() {
                ...
                // For integers, we don't have NaN, so we check if it's 0 (which is what we initialized with for "NaN" in test_strides_inner)
                // Wait, test_strides_inner initializes with F::nan().
                // For i32, F::nan() is 0.
                // So we expect 0.
                if *elt != F::nan() {
                     assert_eq!(*elt, F::nan(),
                        "Element at index={} ({}, {}) should be NaN/Zero, but was {:?}\n\
                        c1: {:?}\n",
                    index, i, j, elt,
                    c1);
                }
